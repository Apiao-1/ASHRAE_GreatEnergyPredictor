...
if __name__ == '__main__':
    ...
    if LOCAL_TEST:
        df_train = pd.read_csv(path_train)[:RECORD_LIMIT]
        df_test = pd.read_csv(path_test)[:RECORD_LIMIT]
        building = pd.read_csv(path_building)[:RECORD_LIMIT]
        weather_train = pd.read_csv(path_weather_train)[:RECORD_LIMIT]
        weather_test = pd.read_csv(path_weather_test)[:RECORD_LIMIT]
    else:
    ...
    df_train = df_train[~((df_train.site_id == 0) & (df_train.meter == 0) & (df_train.building_id <= 104) & (
    ...
    # train：16年1.1-12.31，test:17.1.1-18.12.31
    ...
    df_building_meter_hour = df_train.groupby(["building_id", "meter", "hour"]).agg(
        mean_building_meter=("log_meter_reading", "mean"),
        median_building_meter=("log_meter_reading", "median")).reset_index()
    ...
    all_features = [col for col in df_train.columns if
                    col not in ["timestamp", "site_id", "meter_reading", "log_meter_reading"]]
    ...
    for site_id in range(16):
        ...
        for fold, (train_index, valid_index) in enumerate(kf.split(X_train_site, y_train_site)):
            ...
            model_lgb = lgb.train(params, train_set=dtrain, num_boost_round=1000, valid_sets=watchlist, verbose_eval=False,
